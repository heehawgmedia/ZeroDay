...
class CISSP_D1P1(Scene):
    ...
    def _tech_bg(self) -> VGroup:
        ...
        dots = VGroup(*[
            Dot([-8.8 + c*0.95 + (0.47 if r%2 else 0), -4.0 + r*0.68, 0],
                radius=0.022, color=BLUE).set_opacity(rng.uniform(0.03, 0.09))
            for r in range(13) for c in range(20)
        ])
        corners = [((-6.8, 3.6),(1,-1)), ((6.8, 3.6),(-1,-1)),
                   ((-6.8,-3.6),(1, 1)), ((6.8,-3.6),(-1, 1))]
        brackets = VGroup(*[
            VGroup(
                Line([cx,cy,0],[cx+dx*0.7,cy,0], stroke_width=1.2, color=BLUE, stroke_opacity=0.18),
                Line([cx,cy,0],[cx,cy+dy*0.7,0], stroke_width=1.2, color=BLUE, stroke_opacity=0.18),
            )
    ...
    def s10_practice_questions(self):
        ...
        elapsed = 0.6
        for i, (stem, a, b, c, answer) in enumerate(questions):
